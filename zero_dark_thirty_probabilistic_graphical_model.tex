...
\begin{tikzpicture}[x=1.7cm,y=1.8cm]
  ...
  % More nodes
  \node[latent, right=of X-f] (sigma)  {$\epsilon$}; %
  \node[const, below=of sigma]  (sigma_0) {$\Sigma$}; %
  ...
  \node[const, below=1.0of X]{$p(\mathcal{X} | \mathcal{T}, \Sigma, \Theta, \Phi) = p(\mathcal{T})p(\mathcal{X})p(\epsilon)p(\Theta)p(\Phi)$} ;

  \node[const, below=1.5of X]{$\mathcal{X} = \mathcal{T} + \epsilon$} ;

  \node[const, below=2.0of X]{$\epsilon \sim \mathcal{N}(0, \Sigma)$} ;
